    ...
    def commit_insert_session(self):
        ...
        with lock_db:
            ...
            for k_codec in self._codec.keys():
                c.execute(
                    "insert into Codec ( Id , Name ) values ( ? , ? ) ",
        ...
        with lock_db:
            ...
            if album_sha1 != None:
                ...
                if len(rq) == 0 and not album_sha1 in [k for k in self._albums.keys()]:
                    ...
                    album_id = [
                        self._albums[k]["Id"]
                        for k in self._albums.keys()
                        if k == album_sha1
                    ][0]

            self._tracks[track_sha1] = {
                "id": self._id_track,
                "sha1": track_sha1,
                "title": title,
                "dr_id": dr_id,
